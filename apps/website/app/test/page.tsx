...
function ProposalCard(
  props: {
    ...
    | { type: "markdown"; preview: string; content: string }
  ...
  return (
    <ToggleModal
      ...
      value={props.id}
      className={twMerge(
        "flex flex-col gap-2 bg-grey-800 hover:bg-grey-600 transition-colors rounded-xl overflow-hidden w-full h-full group aspect-[4/3] p-4",
        props.className
      )}
    >
      <p className="text-white font-bebas-neue text-2xl line-clamp-2 flex-shrink-0 leading-tight /h-[2lh]">
        {props.title}
      </p>
      {props.type === "markdown" && (
        <img
