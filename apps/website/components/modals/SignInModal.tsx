  ...
  useEffect(() => {
    if (user) {
      if (user.farcaster?.displayName && user.farcaster?.pfp) {
        setImage(user.farcaster.pfp);
        setName(user.farcaster.displayName);
      ...
      if (user.discord?.username) {
        setImage(
          `https://api.cloudnouns.com/v1/pfp?text=${user.discord.subject}&background=1`
        );
        setName(user.discord.username.replace("#0", ""));
        return;
      }

      if (user.twitter?.username && user.twitter?.profilePictureUrl) {
        setImage(user.twitter.profilePictureUrl);
        setName(user.twitter.username);
        return;
      }
    }
  }, [user]);
