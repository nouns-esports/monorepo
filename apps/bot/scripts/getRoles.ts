...
export const getNexus = async (input: { user: User; inServer: boolean }) => {
  ...
  const now = new Date();
  const threeMonthsAgo = new Date(now.getTime() - 1000 * 60 * 60 * 24 * 30 * 3);
...
for (const user of users) {
  if (!user.discord?.subject) continue;
